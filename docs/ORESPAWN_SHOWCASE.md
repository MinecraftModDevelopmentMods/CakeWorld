...
settings cannot drift. Provider revision 31 currently packages the two
...
| ID | OreSpawn capability | CakeWorld example | Placement | Slice | Verification expectation |
|---|---|---|---|---|---|
| OS-089 | Provider-owned multi-dimension fluid deposit | Five stable rules declare jam, custard, caramel, and syrup in the Overworld plus Hot Fudge in the Nether. | Main | 2/4 | Revision 31 still bakes all five definitions across two dimensions. Earlier differential-world and reload evidence retained the Hot-Fudge output; the Jam-Grotto fixed fresh/reload survey independently retained 1,960 managed Jam cells in the allowed Cookie-Forest region. |
...
| OS-095 | Fluid host blocks and tags | Restrict Jam through explicit Wafer Rock plus the metamorphic host family, and Custard through the edible-host tag. | Main | 2/4 | Revision 31 retains both Jam host forms, the rules bake, and generated reservoirs replace eligible geology; strict negative-host proof resumes when the public API exposes a compiled acceptance predicate or deterministic deposit fixture. |
...
verification claims and uses the covered-fluid-deposit path for Jam. Spring
verification resumes against the released OreSpawn version containing that
fix.
