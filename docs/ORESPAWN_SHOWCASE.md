...
settings cannot drift. Provider revision 36 currently packages the two
...
`4.0.2` beta from CurseForge project `245586`, file `8536302`. The pinned raw
artifact has SHA-256
`532035383acf410babaa52101a814ced40fd3233fbf1c2e6b6ca7ce8c627a9e2`.
...
| ID | OreSpawn capability | CakeWorld example | Placement | Slice | Verification expectation |
|---|---|---|---|---|---|
| OS-034 | Forge biome registration helper | Start simple biomes with `copyAndRegister`; Gingerbread Hearthlands, Cookie Crumb Forest, Peppermint Pinewoods, Gummy Jungle, Caramel Bogs, Sherbet Dunes, Candy-Cane Badlands, Marshmallow Peaks, Ice-Cream Tundra, Waffle Plateaus, Cupcake Gardens, Liquorice Darkwood, Lollipop Orchards, Popcorn Prairie, Soda Ocean, Custard Coast, Jellybean Archipelago, Rock-Candy Caverns, Jam Grottoes, Nougat Depths, Fudge Wastes, Burnt-Toffee Deltas, Cinnamon Ember Groves, Black-Liquorice Labyrinths and Treacle Soul Valleys demonstrate copying complete Plains, Forest, Snowy-Taiga, Bamboo-Jungle, Swamp, Desert, Badlands, Jagged-Peaks, Snowy-Plains, Savanna-Plateau, Sunflower-Plains, Dark-Forest, Birch-Forest, Plains, Ocean, Beach, Mushroom-Fields, Dripstone-Caves, Lush-Caves, a second deliberately distinct Dripstone-Caves definition, Nether Wastes, Basalt Deltas, Crimson Forest, Warped Forest and Soul Sand Valley while replacing climate, ambience and spawn roles. Use `blankAndRegister` only for intentionally complete advanced definitions. | Main | 1/2/3/4 | Direct registry tests prove the twenty-five listed copied climates, including all five Nether outputs at `2.0/0.0`, Nougat Depths `0.6/0.5`, Jam Grottoes `0.7/0.9`, Rock-Candy Caverns `0.8/0.4`, Soda Ocean `0.5/0.5`, Custard Coast `0.8/0.4` and Jellybean Archipelago `0.9/1.0`; each retains its intended inherited source roles while substituting exact CakeWorld creatures. Fixed-seed generation selects these copied biomes through the live three-dimensional overlay; each slice separately proves exact-save persistence. |
...
| OS-102 | Surface-before-child-finalizer ordering | OreSpawn's terrain surface replacement must complete before copied child-mod `TOP_LAYER_MODIFICATION` features that author or repair final landmark cells. CakeWorld deliberately uses late, idempotent finalizers for structures whose terrain-facing cells may otherwise be damaged by generation order. Structural landmark blocks must also be excluded from OreSpawn host tags when they must never become managed deposits; ordering alone is not a resource-protection contract. Burnt-Toffee Deltas exposed a separate copied-feature seam: inherited Basalt columns and blobs can cross into a neighbouring chunk after that chunk's final feature pass. A delayed unload or save-time palette is unsafe because asynchronous chunk storage may already be serialising the chunk. CakeWorld instead filters both Basalt-column and both Basalt/Blackstone-blob features from the Deltas, filters cross-biome Nether Gravel generation from all five converted Nether biomes, installs four native column/blob features that author the structural materials directly, and lets the provider-owned surface supply Crunchy Toffee Ash. Black-Liquorice terrain/tangles and Treacle Soul terrain/flats likewise use provider-owned surfaces plus native features, so neither biome needs a post-generation repair or persistence marker. | Main | 2/3/4/5 | Published beta file `8525123` restored the complete Rock-Candy Crystal Mine, Cookie Grove, Crater Kitchen and other formerly damaged landmarks in a 204-test fresh-world matrix, and same-save reload retained their player-edit sentinels. Rock-Candy Caverns add a stronger direct/fresh/reload example: Cut Rock Candy is craftable but not an edible ore host, so its Geode-Bridge rails and arches survived when ordinary Rock Candy did not. Direct tests prove all five converted Nether biomes exclude the Gravel source, while the copied Deltas biome also excludes four Basalt/Blackstone features and includes all four native structural equivalents. Fresh/reload fixed-seed evidence retains exact Treacle Causeway counts with its Brick edit, 72 independent contained Syrup cells, zero literal soul terrain, exact Black-Liquorice authored counts with its Brick edit, zero Warped-source residue, exact native-Deltas `1,613/41,634/52,734/6,563/0` Ash/Burnt-Sugar/Pillar/Hot-Fudge/literal-source counts and the Deltas Brick sentinel. One naturally generated Marshmallow Cloud Bridge was still incomplete in earlier runs, so the wider ordering contract remains open. Re-enter full acceptance only when a representative fresh/reload matrix retains every late-authored scene and player edit together. |
| OS-103 | Biome-output vertical selector | **Shelved:** OreSpawn 4.0.2 biome-palette outputs expose climate, similarity, namespace, geome and weight controls, but no minimum/maximum Y selector. Nougat Depths therefore competes honestly with Rock-Candy Caverns for Dripstone-Caves sources at any height; only its landmark and geology evidence are restricted to deep Y. | Main | 2 | Re-enter after the public schema/API and compiled selection path support bounded output Y without registry lookups or allocation in the hot loop. Prove adjacent fixtures above and below each cutoff, fresh/reload persistence, weighted overlap and unchanged delegated-source behaviour before claiming a depth-only biome. |
| OS-104 | Vanilla spring compatibility with child-mod rock | Use Minecraft's real configured Water Spring inside a Wafer-Rock fixture, with ordinary Bricks as an ineligible negative control. CakeWorld calls no OreSpawn implementation package; this is a public behavioural integration test of OreSpawn's baked host extension. | Main | 2/4 | The direct test requires the loaded OreSpawn version to be exactly `4.0.2`, proves the real `minecraft:spring_water` configured feature succeeds in the valid Wafer-Rock shell and refuses Bricks, then fixed-seed fresh generation creates the source at `28672,40,28672`. Same-save reload retains the source, complete Wafer shell and a player Brick sentinel. |
...
| ID | OreSpawn capability | CakeWorld example | Placement | Slice | Verification expectation |
|---|---|---|---|---|---|
| OS-089 | Provider-owned multi-dimension fluid deposit | Five stable rules declare jam, custard, caramel, and syrup in the Overworld plus Hot Fudge in the Nether. Revision 36 admits Fudge Wastes, Burnt-Toffee Deltas, Cinnamon Ember Groves, Black-Liquorice Labyrinths and Treacle Soul Valleys to the same Hot-Fudge rule without creating a second definition. | Main | 2/4 | Revision 36 still bakes all five definitions across two dimensions. Fixed fresh/reload Fudge-Wastes evidence retained 42,264 Hot-Fudge cells outside the authored fountain; Burnt-Toffee evidence independently retained 6,563 Hot-Fudge cells inside the Deltas audit; Black Liquorice retained 12,257 cells and Treacle Soul Valleys retained 2,281 cells in identical fresh/reload audits. The Cinnamon and all five exact biome-filter contracts are direct-tested. The Jam-Grotto survey retained 1,960 managed Jam cells in the allowed Cookie-Forest region. |
...
| OS-095 | Fluid host blocks and tags | Restrict Jam through explicit Wafer Rock plus the metamorphic host family, Custard through the edible-host tag, and Hot Fudge through volcanic edible hosts. | Main | 2/4 | Revision 36 retains the Jam host forms and volcanic Hot-Fudge hosts across all five finished Nether biomes, the rules bake, and generated reservoirs replace eligible geology; strict negative-host proof resumes when the public API exposes a compiled acceptance predicate or deterministic deposit fixture. |
| OS-096 | Fluid biome filters | Start Jam only from Cookie-Forest chunks and exclude Syrup starts from Soda-Ocean chunks. OreSpawn 4.0.1 evaluates the Jam filter from the chunk's surface-biome sample, so this is deliberately not described as a Jam-Grotto underground-cell filter. | Main | 2/3 | A separate fixed-seed survey found 100 allowed Cookie-Forest chunks and 1,960 managed Jam cells; fresh/reload retained the result. Final bodies may cross underground biome-cell boundaries after an accepted surface-biome start. |
| OS-097 | Fluid geome weights | Bias covered deposits by flavour geology; Jam declares Cocoa Basin `3.0`, Wafer Shelf `1.0`, Peppermint Fold `0.5` and Rock-Candy Uplift `0.5`. Custard remains the current ratio-measured Wafer-Shelf example. | Main | 2/4 | Direct tests retain every Jam weight and fixed fresh/reload worlds retain managed Jam output. The 1,960-cell Jam observation is not presented as proof of the geome ratio. Earlier Custard evidence measured 1,850 blocks across 51 Wafer chunks versus 250 across 30 Cocoa chunks, with solid envelopes intact. |

OreSpawn `4.0.1` could fail to generate springs when their neighbouring blocks
were non-vanilla. Published beta `4.0.2` is now pinned and `OS-104` verifies
...
OreSpawn 4.0.2 supports only `zone.moddev.mc.orespawn.api`; that API exposes
