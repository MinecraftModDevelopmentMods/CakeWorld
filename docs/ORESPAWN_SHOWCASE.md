...
| OS-032 | Biome rules and biome-dictionary matching | Bias geomes using exact CakeWorld biome IDs plus Forge COLD and HOT dictionary names; retain an unmatched MUSHROOM control. Peppermint Pinewoods combines an exact Peppermint-Fold/Rock-Candy rule with accurate COLD/SNOWY/CONIFEROUS dictionary identity. | Main | 2 | The active profile retains both rule forms, registered types match only intended biomes, fixed-seed samples observe both dictionary-biased geomes, and the exact profile survives reload. Revision 14 fresh/reload sampling kept all 57 Pinewoods columns inside Peppermint Fold. |
...
| ID | OreSpawn capability | CakeWorld example | Placement | Slice | Verification expectation |
|---|---|---|---|---|---|
| OS-034 | Forge biome registration helper | Start simple biomes with `copyAndRegister`; Gingerbread Hearthlands, Cookie Crumb Forest, and Peppermint Pinewoods demonstrate copying complete Plains, Forest, and Snowy-Taiga definitions while deliberately replacing climate and ambient additions. Use `blankAndRegister` only for intentionally complete advanced definitions. | Main | 1/3 | Direct registry tests prove Hearthlands retains its `0.85/0.55` climate and chime, Cookie Forest retains `0.7/0.8` and its quiet canopy rustle, and Pinewoods retains `-0.2/0.7`, its branch chime, exact snowy-taiga spawn weights, and explicit content boundaries. |
...
| OS-044 | Weighted output biomes | Gingerbread Hearthlands enters the Overworld-land palette at weight `2.5`, alongside common Candy Plains and above the `2.0` Cookie Forest, `1.5` Peppermint Pinewoods, and less-common Marshmallow Peaks; later Cupcake Gardens or Jellybean islands become the rare classes. | Main | 3 | The revision-14 profile retains each weight and a large fixed-seed sample falls within defined tolerance for each populated weight class. |
| OS-045 | Similar-biome matching | Gingerbread Hearthlands maps optionally to Plains, Sunflower Plains, and Meadow; Cookie Forest maps to Forest, Flower Forest, and Birch Forest; Peppermint Pinewoods maps to Snowy Taiga, Taiga, and Old-Growth Pine Taiga. Each demonstrates several present hints without making any a hard dependency. | Main | 1/3 | Revision 14 retains all three hint sets with empty required-hint lists; accepted fresh/reload fixed-world sampling observed 2,642 Hearthlands, roughly 3,470 Cookie-Forest, and 5,329 Pinewoods columns. A missing optional-hint fixture remains for the Sampler. |
...
| OS-047 | Temperature and downfall ranges | Gingerbread Hearthlands demonstrates a temperate selector (`0.4..1.2` temperature, `0.25..0.8` downfall); Cookie Forest narrows the wet edge (`0.15..1.5`, `0.55..1.0`); Peppermint Pinewoods supplies the first cold/wet window (`-2.0..0.3`, `0.35..1.0`). Later Sherbet Dunes and Gummy Jungle add dry, hot, and tropical extremes. | Main | 1/3 | Revision 14 retains all three selector contracts and fresh-world selection observes every biome; later boundary fixtures at min/max and just outside them remain. |
| OS-048 | Surface top block | Use icing turf, sponge, crumb sand, fudge crust, and meringue signatures. Gingerbread Hearthlands uses gravity-affected Biscuit Crumbs, Cookie Forest uses Chocolate Sponge beneath its authored grove, and Peppermint Pinewoods uses Icing Layers beneath its striped clearing ecology. | Main | 1/3/4/5 | Revision-14 fresh/reload sampling retained 1,859 Hearthlands crumb tops, 1,270/1,268 Cookie-Sponge tops, and 1,152 Pinewoods Icing-Layer tops, alongside green Candy, Marshmallow, Fudge, and Meringue controls. |
| OS-049 | Surface filler block and depth | Put Chocolate Sponge four blocks below the Hearthlands crumb surface, five blocks below Cookie Forest, and five blocks of Peppermint Rock below Pinewoods icing, with deeper biome-specific edible layers elsewhere. | Main | 1/3/4/5 | Revision 14 retains every 4/5-block depth; accepted fresh/reload sampling exposed 9,252 Hearthlands Sponge cells, 8,065/8,054 Cookie-Forest Sponge cells, and a stable 16,768 Pinewoods Peppermint-Rock cells while all other dry/realm controls remained green. |
