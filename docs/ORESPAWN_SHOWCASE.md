...
settings cannot drift. Provider revision 59 packages two adventure templates
...
copied-world retrogen profile, unrelated-source composition plot, public Layer
Cake pattern, and reproducible formation comparison are verified. Every
labelled `OS-001` through `OS-105` capability has now been attempted. The
unsupported cases remain explicitly shelved below rather than being hidden or
misrepresented as working examples.

The published production dependency is Minecraft 1.18.2 OreSpawn
`4.0.16.118021` from CurseForge project `245586`, file `8780769`, with raw
SHA-256 `76fd6140fe85c3d9eea0321a0441d352a0b2643390a1090bbb1f70fe104c5717`.
The published JAR is byte-for-byte identical to the validated candidate from
commit `4a9f9447293f4f6c088ef498a6f91216ba4c77f7`.
...
| OS-003 | `cakeworld:sampler_platter` | Never auto-selected; chosen explicitly by developers or pack makers. | **Verified:** revision 59 retains nine biome plots, exact-ID and dictionary ore-filter probes, the profile-wide `sky_v1` extreme-formation comparison, an installed-output alias, three-layer flat bedrock, inert retrogen fixtures, and the active `cakeworld:layer_cake` custom-pattern rule while inheriting the adventure geology definitions exactly at build time. Against the `4.0.12.118021` candidate, all six focused tests passed fresh and on same-save reload: exact Candy-Plains output remained absent from Cookie Forest and neutral Peppermint, dictionary output appeared in Peppermint and vanilla Forest but not DENSE Cookie Forest or neutral Candy Plains, aliases and bedrock remained exact, and Layer Cake retained its geometry and weighted ordering. Separate isolated cycles verify bounded retrogen, the all-`tiny` profile, unrelated-source composition, and reproducible formation exports. |
...
| OS-006 | Forge IMC provider submission | **Verified:** keep packaged JSON as the sole distributable declaration, but conditionally build and enqueue a disabled lifecycle probe plus selector and weighted-host probes during `InterModEnqueueEvent` after an isolated harness removes only the generated runtime resource. | Sampler | 7 | Released OreSpawn froze revision `6001` with exactly three owned ores and no templates. The active world snapshot retained all three `source_provider=cakeworld` rules and exact `known_ores`; mutating returned provider JSON did not affect the built definition. One focused test passed and saved all dimensions. The finalizer restores the current packaged revision and removes the test dimension immediately afterward. |
...
| OS-015 | Automatic template selection and priority | **Verified:** CakeWorld-only fresh worlds select priority-100 `edible_world`, BaseMetals fresh worlds select priority-200 `edible_world_basemetals`, an explicit global default selects the otherwise inert Sampler, and an isolated unbundled fixture gives `alpha_priority_probe` and `zulu_priority_probe` equal priority `777`. | Main/BaseMetals | 1/2 | Released OreSpawn 4.0.6 selected lexical `alpha_priority_probe`, retained provider revision `1501` plus both known IDs, and passed fresh and same-save reload without leaking packaged adventure rules. The fixture is absent from the mod JAR. The advertised tie warning is tracked separately by OS-105. |
...
| OS-017 | Existing-world rule merge and tombstones | **Verified:** an isolated unbundled provider created a revision-1701 world snapshot, CakeWorld then made explicit world-owned edits before installing revision 1702. The upgrade retained edited rock and ore definitions, a disabled rule, a deliberately unassigned ID, a custom rule and an output alias; it merged one new rock and one new ore and retained the removed provider ore with `orphaned_provider: true`. | Sampler | 7 | Fresh baseline, copied-world upgrade and same-save reload each passed against released OreSpawn 4.0.6.118021. The manifest retained the historical known-ID union and current revision/template, every dimension saved, and the revision-1702 profile SHA-256 was byte-identical before and after reload. Both provider fixtures are excluded from the mod JAR. |
...
| OS-020 | Creation-time editor | **Blocked against the `4.0.12.118021` release candidate by OreSpawn #269:** the isolated client opened CakeWorld, displayed the auto-selected template and its biome/material pages, but Done rejected the valid namespaced geome as `Invalid geome: cakeworld:cocoa_basin`. Runtime JSON continues to bake and generate the same key correctly. | Main | 7 | Fix both add-time and final editor validation to accept ResourceLocation-shaped geome IDs, then create fixed-seed world `5059928472718672684` through the UI and prove its saved snapshot plus generated output. Do not weaken CakeWorld's public IDs as a workaround. |
...
| OS-027 | Alternate or legacy geology mode | **Verified:** keep `stable_layers` with average presets in the adventure and expose `sky_v1` only through the explicitly selected Sampler. Because formations are profile-wide, the honest comparison uses separate same-seed world-owned profiles over identical CakeWorld geology rather than claiming adjacent algorithms in one world. | Sampler | 7 | The exact `4.0.16.118021` candidate uses the highest occupied surface block and stable direct quart-biome attribution in both generation and the public sampler. Fixed seed `5059928472718672684` retains exact Sampler `36,517/1,184/7/14/2,491,678,091,116,701,092` vertical transitions/horizontal transitions/distinct rocks/distinct geomes/unsigned signature; the stable adventure baseline is independently `5,456/643/7/5/15,554,542,676,638,589,477`. |
...
| OS-032 | Biome rules and biome-dictionary matching | **Verified against the `4.0.12.118021` candidate; OreSpawn #262 has green CakeWorld proof:** retain additive exact CakeWorld biome weights and Forge dictionary weights. | Main | 2/4/5 | Fresh and same-save reload sampled Marshmallow Peaks with both Peppermint Fold (`118`) and Rock-Candy Uplift (`95`), and sampled Fudge Wastes with Fudge Mantle (`5`). The stable-ID runtime path now retains dictionary contributions instead of collapsing to exact rules only. |
...
| ID | OreSpawn capability | CakeWorld example | Placement | Slice | Verification expectation |
|---|---|---|---|---|---|
| OS-034 | Forge biome registration helper | **Verified:** all thirty-five production biomes use `copyAndRegister` for readable source-compatible definitions. An opt-in registry-only `cakeworld_fixture:blank_biome_laboratory` demonstrates `blankAndRegister` by explicitly supplying every required builder field with deliberate empty base spawn and generation settings. | Main/Sampler | 1/2/3/4/5/7 | Direct registry tests prove all copied climates. A focused fresh runtime proved the complete blank definition, empty mob/carver contracts, exact later composition with only `orespawn:biome_surfaces` and `orespawn:stone_replacer`, ordinary `edible_world` selection, and no fixture ID in the active profile or possible biome outputs. A separate ordinary fresh-world test proved the fixture is not registered without its switch. |
| OS-035 | Replace placement mode | **Verified:** the automatically selected main adventure keeps five enabled palettes across all three vanilla dimensions, each with `mode=replace`, `scope=all`, full coverage, zero fallback and a CakeWorld output-namespace exclusion. An isolated owner supplies a fixed unrelated Overworld biome before released OreSpawn wraps it. | Main | 1/3/4/5 | Fresh and same-save reload each reproduced exact fixed-seed signatures across 264,196 positions. The unknown source mapped all 132,098 Overworld surface/deep samples to the only similarity-unrestricted output, Candy Plains; vanilla-backed Nether and End each exposed all seven current outputs. Every result was a CakeWorld biome, the fixture never leaked, the saved-profile SHA-256 stayed unchanged, and an ordinary fresh world kept the fixture unregistered. |
...
| OS-045 | Similar-biome matching | **Verified, including the missing-optional case:** thirty-three adventure outputs retain optional exact hints; Candy Plains and Meringue Islands are deliberately generic. The Sampler Fudge output lists both real `minecraft:nether_wastes` and absent `cakeworld:missing_optional_sampler_source`. | Main/Sampler | 1/2/3/4/5/7 | Released 4.0.6 silently ignores the missing optional member, keeps the resolved Nether-Wastes match active, and fresh/reload each convert the same 1,511 samples. The strict companion remains isolated under OS-046. |
...
| OS-047 | Temperature and downfall ranges | **Verified:** all thirty-five production outputs retain their source-inclusive or deliberately narrow climate windows, comprising seventeen distinct range tuples. | Main/Sampler | 1/2/3/4/5/7 | The generated diagnostic first proves all 35 packaged IDs map into the expected 17 ranges. Released OreSpawn then passed 102 isolated checkerboard cases: 34 exact min/min and max/max corners selected, while the 68 nearest representable values beyond each individual temperature/downfall boundary delegated unchanged. An ordinary fresh world kept the diagnostic namespace unregistered. |
...
| OS-104 | Vanilla spring compatibility with child-mod rock | Use Minecraft's real configured Water Spring inside a Wafer-Rock fixture, with ordinary Bricks as an ineligible negative control. CakeWorld calls no OreSpawn implementation package; this is a public behavioural integration test of OreSpawn's baked host extension. | Main | 2/4 | The direct test now runs against `4.0.12.118021`, proves the real `minecraft:spring_water` configured feature succeeds in the valid Wafer-Rock shell and refuses Bricks, then fixed-seed fresh generation creates the source at `28672,40,28672`. Same-save reload retains the source, complete Wafer shell and a player Brick sentinel. |
| OS-105 | Equal-priority auto-selection setup warning | **Shelved against released OreSpawn 4.0.6.118021:** the documented lexical winner is correct, but the setup warning is declaration-order-dependent. When the lexical winner is declared first, the later equal-priority loser does not enter OreSpawn's warning branch and the tie is silent. | Sampler | 7 | Re-enter after a published OreSpawn release warns for both declaration orders. Repeat isolated fresh runs with `alpha,zulu` and `zulu,alpha`, require the same lexical winner and one actionable warning in each, then retain same-save selection and ordinary-world controls. |
| OS-051 | Ceiling surface block | **Verified against the `4.0.12.118021` candidate:** Black-Liquorice Labyrinths declare `ceiling_block=cakeworld:black_liquorice_stone`, producing a player-facing edible underside rather than changing the inaccessible top of the Nether roof. | Main | 2/4 | Fresh and reload each found 4,078 Black-Liquorice cells among 4,624 target downward-exposed roof faces. Every miss was a later declared OreSpawn geology/ore output; other-biome and roof-top leakage were both zero. |
...
| OS-055 | Snow material | **Blocked against the `4.0.12.118021` candidate by OreSpawn #267:** the profile retains `snow_block=cakeworld:icing_layer`, but the public-route black-box regression posts Forge's ordinary `ChunkEvent.Load` for a loaded ServerLevel fixture and observes `snow=false`. Literal vanilla Snow remains. | Main | 1/3 | Fix the normal event/runtime route, make `cakeworld_weather_materials` green, then prove naturally formed snow conversion in fresh and reload worlds plus another OreSpawn consumer. |
| OS-056 | Ice material | **Blocked with OS-055 by OreSpawn #267:** the same candidate regression reports `ice=false`; literal vanilla Ice remains instead of Frozen Lemonade. | Main | 1/3 | Re-enter with OS-055 after the public route is fixed and integrated-world evidence passes. |
...
| OS-063 | OS3-style ordinary-dimension selector | **Verified:** the IMC selector probe declares `orespawn:all_except_nether_end` with a Stone host, CakeWorld's registered point pattern, Y `32..47`, 64 attempts and quantity `1`. A test-only flat ordinary dimension supplies a deterministic Stone band. | Sampler | 7 | Two independent fresh directories reproduced exactly 57 Sprinkle outputs in fixed seed/chunk `512,512` of `cakeworld:sampler_pantry`, with zero in Nether and End. The spreading-cluster totals `1,936` and `1,581` are retired historical evidence rather than active assertions: single-chunk cluster counts depend on neighbouring generation order, and the older result also predated OreSpawn's corrected feature stage. The dimension JSON is removed after the run and absent from the JAR. |
| OS-064 | Explicit dimension override | **Verified:** the same selector-owned rule adds an explicit disabled `minecraft:overworld` entry with zero frequency. | Sampler | 7 | The fixed Overworld chunk contains zero Sprinkle output while the otherwise equivalent ordinary fixture dimension contains exactly 57; Nether and End remain excluded. Active-profile JSON retains both the broad selector and one exact explicit override. |
...
| OS-073 | Host geology family | Production resources use family hosts; optional Starsteel demonstrates an End rule accepting sedimentary, metamorphic and intrusive families. Integrated output is green, but strict sampler attribution is **blocked by OreSpawn #266**. | Main | 2/4/5 | Revision 59 narrows Starsteel to Starlight Sugar Fields, Y `0..112`, frequency `8`; BaseMetals fresh/reload retained `33` natural cells in the focused biome and `40` in its compatibility scan. The independent strict Overworld sampler saw 3,098 family-hosted outputs and only two mismatches, at `13,-17,-32` and `26,-9,-33`, while 2,264 nonreplaceable Candy-Glass controls stayed untouched. Do not call the per-cell sampler contract verified until #266 is green. |
...
| OS-076 | Weighted host blocks and tags | **Verified:** an isolated IMC diagnostic assigns weight `0.75` to `minecraft:deepslate_ore_replaceables` and `0.25` to the explicit `minecraft:stone` block. Its conditional public-registry point pattern tries exactly one host candidate per attempt so built-in pattern budget filling cannot dilute the acceptance probabilities. | Sampler | 7 | Across a fixed 8x8 chunk grid beginning at `512,512`, equal 32-block source bands must produce at least 1,000 deepslate-tag and 250 Stone-block Fizzy Pearls with a 2.5:1 through 3.75:1 ratio. Both independent fresh selector runs observed `1,493/422`, approximately `3.538:1`. This statistical contract replaces incidental exact totals; Overworld, Nether and End controls remain exactly zero, and the rule, pattern registration and dimension remain inactive outside the explicit fixture runtime. |
| OS-077 | Biome include/exclude IDs | **Verified against the `4.0.12.118021` candidate; OreSpawn #263 has green CakeWorld proof.** A Sampler-only exact-ID rule admits Candy Plains, excludes Cookie Forest, and rejects neutral Peppermint Pinewoods. | Sampler | 2/7 | Fresh/reload generated `148` exact outputs in Candy Plains and zero in both excluded Cookie Forest and neutral Peppermint. The production Fizzy-Pearl Soda-Ocean include is independently green. |
| OS-078 | Forge biome-dictionary filters | **Runtime JSON verified against `4.0.12.118021`; fluent IMC authoring remains blocked by OreSpawn #268.** The Sampler rule includes FOREST, excludes DENSE, and leaves neutral Candy Plains alone. | Sampler | 7 | Fresh/reload generated `189` dictionary outputs in Peppermint Pinewoods and `188` in vanilla Forest, with zero in DENSE Cookie Forest and neutral Candy Plains. `WorldgenProvider.OreDimension.Builder` still exposes no biome include/exclude/dictionary methods, so this complete example currently requires JSON. |
| OS-079 | Geome filters and weights | Favour flavour-specific deposits. The representative integrated Mint control gives Peppermint Fold weight `5.0` and Cocoa Basin weight `0.0`; optional Starsteel adds positive Meringue-Crust `4.0` and Rock-Candy-Uplift `6.0` weights. | Main | 2/5 | The Mint fixed world retained its baked weights, placed 123 crystals on explicit Peppermint hosts with zero sampler violations, and kept Cocoa Basin excluded. The tuned BaseMetals Starlight world generated `33` natural Starsteel cells and retained them on reload. These observations prove integrated weighted-rule output without treating bounded counts as a selection-ratio measurement. |
...
| OS-084 | Cluster pattern | **Verified:** production Sprinkle Clusters retain the built-in cluster pattern, while the compiled-pattern audit isolates its geometry from dimension routing and chunk-generation order. | Main/Sampler | 2/7 | Former selector settings -- quantity `32`, spread `8`, vertical spread `4`, node size `4` and seed `1978` -- consume the full 32-placement budget, stay within the calculated squared-distance boundary `187` (observed maximum `81`) and reproduce sorted-coordinate signature `13,987,560,339,079,957,851` unsigned in the independent full run and final combined benchmark/full run. |
| OS-085 | Under-fluids pattern | **Verified against the `4.0.12.118021` candidate:** Fizzy Pearls use the public `orespawn:underfluids` compiler with registered Lemonade, Soda-Ocean include, family hosts and Wafer-Shelf weighting. Authored Wafer Reef treasures are recognised and excluded. | Main/BaseMetals | 2/3 | Provider revision 59 raises frequency to `1.0` so the larger compatibility rule set remains reliable. The complete normal fresh/reload pair found `41/40` total Pearls and `13/13` directly beneath Lemonade after excluding `10/9` authored reef treasures. The complete BaseMetals pair found `26/25` total and `5/5` directly beneath Lemonade after excluding `10/9` authored treasures. |
| OS-086 | Public custom-pattern registry | **Verified:** register `cakeworld:layer_cake` unconditionally as an experimental public `OrePatternType`; its compiler captures primitive settings once and returns a small allocation-free placer. | Sampler | 7 | Direct registry lookup and codec decode produced an exact 39-cell, three-layer disc; integrated OreSpawn generation baked and placed the registered pattern in two independent fresh Sampler worlds. |
| OS-087 | Custom pattern settings JSON | **Verified:** give `layer_cake` bounded `layers`, `radius`, `thickness`, and `layer_gap` settings, then use OreSpawn's standard weighted outputs for Mint Crystal, Fizzy Pearl and Rich Sprinkle flavours. | Sampler | 7 | Settings `3/2/1/1` decoded to exact 13-cell discs at relative Y `-2/0/2`; radius `0` failed with a useful codec bound/path, and the active snapshot retained exact settings plus `3:2:1` output weights. |
| OS-088 | Custom-pattern performance decision | **Verified — keep:** retain `layer_cake` because it is readable, bounded and no slower than the built-in precision comparison in the accepted focused measurements. | Sampler | 7 | Fresh/reload 20,000-placement comparisons retained exactly 780,000 `tryPlace` calls per pattern; final-source Layer Cake measured `0.746x` and `0.391x` the precision time. The same saved 4x4 survey retained `1,062/678/352` Mint/Fizzy/Rich cells with the intended ordering; two earlier independent fresh worlds also retained the ordering. |
...
were non-vanilla. Published release `4.0.16.118021` is now pinned and `OS-104` verifies
...
| OS-099 | Retrogen | **Verified:** package one disabled Sprinkle rule marked for retrogen beside one disabled non-retrogen Fizzy-Pearl control, then enable both and revision `5301` only in an ignored copied Sampler save. The ordinary adventure and both packaged retrogen fixtures remain inert; the unrelated Layer Cake diagnostic stays active. | Sampler | 7 | Baseline created exact 256-block eligible Biscuit and ineligible Brick layers in old chunks. Apply produced exactly 64 Sprinkles plus 192 Biscuit, zero Fizzy Pearls, an unchanged 256-block Brick control and intact sentinels. Same-save reload retained those exact counts with `force: false`, proving the revision marker prevented a second pass. A separate ordinary fresh world still selected `edible_world`; packaged Sampler fresh/reload baked only the non-retrogen Layer Cake rule. |
...
| OS-101 | Extreme formations | **Verified:** revision 59 retains the maximum-variation bounded custom case beside the separately verified normal adventure. An isolated generated provider override adds the opposite named-preset case by selecting `tiny` for all five controls without changing the packaged three-template contract. An opt-in GameTest export renders a full-resolution vertical section, top-down slice, rock legend, seed and asserted metrics from the public sampler. | Sampler | 7 | OreSpawn `4.0.16.118021` intentionally classifies at the highest occupied surface block and uses the same direct quart-biome cell for generation and sampling. The extreme case is exact `36,517/1,184/7/14/2,491,678,091,116,701,092`; the independently rerun stable case is `5,456/643/7/5/15,554,542,676,638,589,477`, and the minimum-preset case is `32,296/1,624/8/14/6,569,562,011,636,171,901`. |
...
| OS-TEST-004 | `sampler_platter` explicit selection | **Verified:** packaged and active-profile tests prove explicit selection, non-automatic metadata, canonical shared geology definitions, custom and minimum-preset `sky_v1` formations plus reproducible visual comparison, augment, namespace filters, optional/required similarity, output aliases, flat bedrock, bounded copied-world retrogen, the public compiled Layer Cake pattern, exact three-realm fresh/reload distributions, and independent automatic selection of `edible_world`. Every labelled capability has been attempted; individual unsupported cases remain visibly Shelved with their own evidence and revisit conditions. |
...
bytes completed the final post-maintenance 81-chunk, three-repetition Sky
benchmark at a median 35.394 ms/chunk. After publication, a fresh
CurseMaven-resolved IMC run and a separate fresh full run passed all 274
required tests, saved every world and exited Gradle normally. An earlier
attempt completed the benchmark but had one unrelated Confectioner's
Cottage resident-availability timeout; both a clean non-benchmark full run and
the final combined rerun passed that audit. The earlier accepted 35.838
ms/chunk median remains historical timing evidence. The post-save
`GameTestServer.onServerExit` null-pointer and Gradle hang are resolved;
successful generation without a clean harness exit does not satisfy this
contract.
...
working runtime examples under the `4.0.12.118021` candidate. CakeWorld's
...
placement origin. The narrowed `4.0.12.118021` comparison now leaves only two
family-host mismatches among 3,098 outputs, with exact coordinates recorded in
OreSpawn #266; 2,264 nonreplaceable Candy-Glass controls remain untouched. By
contrast, Mint Crystal's explicit-block rule is fully green with 123 outputs
and zero sampler violations on fresh and reload.

`OS-085` is now green against the `4.0.12.118021` candidate. The exact
Soda-Ocean include survives the dynamic registry boundary, and the compiled
pattern generates independently attributable Pearls in normal and BaseMetals
fresh/reload worlds. The regression still recognises every complete Wafer Reef
Nursery before subtracting authored treasures, so those scenes cannot satisfy
the OreSpawn assertion accidentally.
...
Only Pearls outside those authored scenes count toward `OS-085` evidence.
...
`OS-032` is green against the `4.0.12.118021` candidate. CakeWorld registers
dictionary types during common setup, and the stable-ID runtime fallback now
merges exact and dictionary weights. Fresh and same-save reload both retain
Peppermint Fold and Rock-Candy Uplift in Marshmallow Peaks, with Fudge Mantle
also present in Fudge Wastes.
