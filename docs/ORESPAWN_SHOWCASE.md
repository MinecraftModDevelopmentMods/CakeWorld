...
settings cannot drift. Provider revision 47 packages two adventure templates
and the explicitly selected diagnostic Sampler. Its augment foundation is
...
| OS-003 | `cakeworld:sampler_platter` | Never auto-selected; chosen explicitly by developers or pack makers. | **In progress:** revision 47 packages and explicitly selects the isolated augment foundation. Fresh and same-save reload runs retained the saved template and exact fixed-seed `66,049/1,414/64,635/50` sample/Candy-Plains/Minecraft/distinct-biome result; a separate ordinary fresh world selected `cakeworld:edible_world`. Extreme formations, aliases, retrogen, flat bedrock, intrusive test rules, and compact visual inspection remain later plots. |
...
| OS-041 | Region sizes | **Partial:** adventure palettes retain their verified broad realm settings and the Sampler foundation adds a live `tiny` 128-block region class. Small through huge side-by-side comparison plots remain. | Main/Sampler | 3/7 | The packaged and active profiles agree on `tiny`; the fixed survey crosses many mixed boundaries. Do not mark the capability complete until all five 128, 256, 512, 1024, and 2048-block classes have labelled boundary evidence. |
...
| OS-TEST-004 | `sampler_platter` explicit selection | **Foundation passed:** packaged and active-profile tests prove explicit selection, non-automatic metadata, the bounded augment plot, exact fresh/reload distribution, and independent automatic selection of `edible_world` in an ordinary revision-47 fresh world. Completion still requires every remaining labelled diagnostic plot. |
