...
settings cannot drift. Provider revision 32 currently packages the two
...
| ID | OreSpawn capability | CakeWorld example | Placement | Slice | Verification expectation |
|---|---|---|---|---|---|
| OS-089 | Provider-owned multi-dimension fluid deposit | Five stable rules declare jam, custard, caramel, and syrup in the Overworld plus Hot Fudge in the Nether. Revision 32 admits both Fudge Wastes and Burnt-Toffee Deltas to the same Hot-Fudge rule without creating a second definition. | Main | 2/4 | Revision 32 still bakes all five definitions across two dimensions. Earlier differential-world and reload evidence retained the Fudge-Wastes output; Burnt-Toffee fixed fresh/reload evidence independently retained 6,563 Hot-Fudge cells inside the Deltas audit. The Jam-Grotto survey retained 1,960 managed Jam cells in the allowed Cookie-Forest region. |
...
| OS-095 | Fluid host blocks and tags | Restrict Jam through explicit Wafer Rock plus the metamorphic host family, Custard through the edible-host tag, and Hot Fudge through volcanic edible hosts. | Main | 2/4 | Revision 32 retains the Jam host forms and Deltas-compatible Hot-Fudge hosts, the rules bake, and generated reservoirs replace eligible geology; strict negative-host proof resumes when the public API exposes a compiled acceptance predicate or deterministic deposit fixture. |
...
OreSpawn `4.0.1` can fail to generate springs when their neighbouring blocks
are non-vanilla. CakeWorld therefore excludes springs from its current
verification claims and uses the covered-fluid-deposit path for Jam. The fix
is expected in OreSpawn `4.0.2`, but that version is not yet published on
CurseForge. Spring verification resumes only after the fixed release is
