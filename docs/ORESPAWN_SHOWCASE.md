...
settings cannot drift. Provider revision 49 packages two adventure templates
and the explicitly selected diagnostic Sampler. Its augment, namespace-filter,
and optional/required similarity plots are verified; the remaining labelled
...
| OS-003 | `cakeworld:sampler_platter` | Never auto-selected; chosen explicitly by developers or pack makers. | **In progress:** revision 49 retains the Overworld augment and namespace plots, then adds a Nether optional/required similarity pair. Fresh and same-save reload reproduced `66,049/1,414/64,635/50` Overworld samples, `4,225/1,511/0/2,714/5` Nether total/Fudge/strict/Minecraft/distinct results, and `4,225/4,225/5/0` End total/Minecraft/distinct/Meringue results. Extreme formations, aliases, retrogen, flat bedrock, intrusive test rules, and compact visual inspection remain later plots. |
...
| OS-038 | `selected_namespaces` replacement scope | **Verified:** the Sampler's Nether plot uses `scope=selected_namespaces` with the explicit `minecraft` allow-list, replace mode, full coverage and zero fallback. | Sampler | 7 | Revision 48 first converted all 4,225 vanilla-source positions. Revision 49 narrows its valid output with optional similarity and consistently converts the 1,511 Nether-Wastes samples while delegating 2,714 other vanilla samples that have no eligible output. The unrelated-mod compatibility fixture remains OS-037's stronger gate. |
...
| OS-TEST-004 | `sampler_platter` explicit selection | **In progress:** packaged and active-profile tests prove explicit selection, non-automatic metadata, augment, namespace filters, optional/required similarity, exact three-realm fresh/reload distributions, and independent automatic selection of `edible_world`. Completion still requires every remaining labelled diagnostic plot. |
