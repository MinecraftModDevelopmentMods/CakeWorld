...
settings cannot drift. Provider revision 55 packages two adventure templates
...
| OS-003 | `cakeworld:sampler_platter` | Never auto-selected; chosen explicitly by developers or pack makers. | **Verified:** revision 55 retains nine verified biome plots, the profile-wide `sky_v1` extreme-formation comparison, an installed-output alias, three-layer flat bedrock, inert retrogen fixtures, and the active `cakeworld:layer_cake` custom-pattern rule while inheriting the adventure geology definitions exactly at build time. Fresh and same-save reload retained the established biome/formation results, baked 512 aliased Sprinkle samples with zero original Burnt-Sugar samples, and retained exact `256/256/256/0/0` floor/ceiling layer counts. Separate isolated cycles verified bounded retrogen, a generated all-`tiny` minimum-preset profile, unrelated-source composition, Layer Cake geometry plus weighted flavour output, and a reproducible labelled formation/readability export without adding a packaged template or production dependency. Every labelled capability has been attempted; unsupported public-contract cases remain separately Shelved and do not make this safe, explicit diagnostic template incomplete. |
...
| OS-020 | Creation-time editor | **Shelved against released 4.0.6.118021:** the editor correctly opens CakeWorld, shows its registered biome/material IDs, and keeps **Biomes & World Materials** available under the no-strata Pack Defaults baseline. However, applying either CakeWorld template is rejected before world creation as `Invalid geome: cakeworld:cocoa_basin`. The released runtime/schema and provider example accept namespaced geome IDs, while `GeologyEditorSession` creation/save validation admits only `[a-z0-9_.-]+` keys. | Main | 7 | Re-enter only after a published OreSpawn release accepts provider-owned namespaced geomes in the creation editor. Then select a CakeWorld template in an unused isolated client directory, create fixed-seed world `5059928472718672684`, and prove the saved snapshot plus generated output match the UI selection. |
...
| OS-063 | OS3-style ordinary-dimension selector | **Verified:** the IMC selector probe declares `orespawn:all_except_nether_end` with a Stone host, clusters pattern, Y `32..47`, 64 attempts and quantity 32. A test-only flat ordinary dimension supplies a deterministic Stone band. | Sampler | 7 | Fixed seed/chunk `512,512` contains exactly 1,581 Sprinkle outputs in `cakeworld:sampler_pantry`; Nether and End each contain zero. The profile has one selector entry and no explicit custom-dimension duplicate. The dimension JSON is removed after the run and absent from the JAR. |
| OS-064 | Explicit dimension override | **Verified:** the same selector-owned rule adds an explicit disabled `minecraft:overworld` entry with zero frequency. | Sampler | 7 | The fixed Overworld chunk contains zero Sprinkle output while the otherwise equivalent ordinary fixture dimension contains exactly 1,581; Nether and End remain excluded. Active-profile JSON retains both the broad selector and one exact explicit override. |
...
| OS-076 | Weighted host blocks and tags | **Verified:** an isolated IMC diagnostic assigns weight `0.75` to `minecraft:deepslate_ore_replaceables` and `0.25` to the explicit `minecraft:stone` block. Its conditional public-registry point pattern tries exactly one host candidate per attempt so built-in pattern budget filling cannot dilute the acceptance probabilities. | Sampler | 7 | Across the fixed 4x4 chunk grid beginning at `512,512`, equal 32-block source bands produced exactly 374 Fizzy Pearls from the weighted deepslate tag and 121 from the weighted Stone block, a 3.091:1 ratio against the declared 3:1 expectation. Overworld, Nether and End controls were zero; the rule, pattern registration and dimension remain inactive outside the explicit fixture runtime. |
...
| OS-101 | Extreme formations | **Verified:** revision 55 retains the maximum-variation bounded custom case beside the separately verified normal adventure. An isolated generated provider override adds the opposite named-preset case by selecting `tiny` for all five controls without changing the packaged three-template contract. An opt-in GameTest export renders a full-resolution vertical section, top-down slice, rock legend, seed and asserted metrics from the released public sampler. | Sampler | 7 | Same-seed public-API surveys retain exact stable `5,393/634/7/5/6,720,209,891,956,171,365`, extreme `36,515/1,176/7/14/2,804,912,711,855,593,311`, and minimum-preset `32,350/1,627/8/14/12,479,179,277,466,877,779` transition/identity signatures. Visual review shows broad coherent stable strata, intentionally shredded extreme output, and dense but recognizable all-`tiny` beds with periodic vanilla-stone gaps. The extreme profile is a useful diagnostic, not a recommended adventure setting; `minimum` names the controls rather than promising the fewest observed transitions. Fresh/reload evidence saved and no OreSpawn change was required. |
