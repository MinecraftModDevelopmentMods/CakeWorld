...
settings cannot drift. Provider revision 34 currently packages the two
...
| OS-102 | Surface-before-child-finalizer ordering | OreSpawn's terrain surface replacement must complete before copied child-mod `TOP_LAYER_MODIFICATION` features that author or repair final landmark cells. CakeWorld deliberately uses late, idempotent finalizers for structures whose terrain-facing cells may otherwise be damaged by generation order. Structural landmark blocks must also be excluded from OreSpawn host tags when they must never become managed deposits; ordering alone is not a resource-protection contract. Burnt-Toffee Deltas expose a separate copied-feature seam: Basalt-Deltas decoration can cross into a neighbouring chunk after that chunk's final feature pass. CakeWorld therefore marks only brand-new Deltas chunks pending, waits for their loaded `3 x 3` neighbourhood, applies its bounded palette once, and persists completion; unmarked chunks loaded from disk are never converted. | Main | 2/3/4/5 | Published beta file `8525123` restored the complete Rock-Candy Crystal Mine, Cookie Grove, Crater Kitchen and other formerly damaged landmarks in a 204-test fresh-world matrix, and same-save reload retained their player-edit sentinels. Rock-Candy Caverns add a stronger direct/fresh/reload example: Cut Rock Candy is craftable but not an edible ore host, so its Geode-Bridge rails and arches survived when ordinary Rock Candy did not. Burnt-Toffee fixed-seed fresh/reload evidence retained identical `1,650/33,921/1,617` Crunchy-Ash/Burnt-Sugar/Pillar counts, zero eligible vanilla source blocks and a player Brick sentinel; the broader terrain audit independently reported empty Nether residuals. One naturally generated Marshmallow Cloud Bridge was still incomplete in earlier runs, so the wider ordering contract remains open. Re-enter full acceptance only when a representative fresh/reload matrix retains every late-authored scene and player edit together. |
...
| ID | OreSpawn capability | CakeWorld example | Placement | Slice | Verification expectation |
|---|---|---|---|---|---|
| OS-089 | Provider-owned multi-dimension fluid deposit | Five stable rules declare jam, custard, caramel, and syrup in the Overworld plus Hot Fudge in the Nether. Revision 34 admits Fudge Wastes, Burnt-Toffee Deltas and Cinnamon Ember Groves to the same Hot-Fudge rule without creating a second definition. | Main | 2/4 | Revision 34 still bakes all five definitions across two dimensions. Fixed fresh/reload Fudge-Wastes evidence retained 42,264 Hot-Fudge cells outside the authored fountain; Burnt-Toffee evidence independently retained 6,563 Hot-Fudge cells inside the Deltas audit, and the Cinnamon direct contract proves the widened exact filter. The Jam-Grotto survey retained 1,960 managed Jam cells in the allowed Cookie-Forest region. |
...
| OS-095 | Fluid host blocks and tags | Restrict Jam through explicit Wafer Rock plus the metamorphic host family, Custard through the edible-host tag, and Hot Fudge through volcanic edible hosts. | Main | 2/4 | Revision 34 retains the Jam host forms and volcanic Hot-Fudge hosts across all three finished Nether biomes, the rules bake, and generated reservoirs replace eligible geology; strict negative-host proof resumes when the public API exposes a compiled acceptance predicate or deterministic deposit fixture. |
