...
| OS-044 | Weighted output biomes | Gingerbread Hearthlands enters the Overworld-land palette at weight `2.5`, alongside common Candy Plains and above the `2.0` Cookie Forest and less-common Marshmallow Peaks; later Cupcake Gardens or Jellybean islands become the rare classes. | Main | 3 | The revision-13 profile retains each weight and a large fixed-seed sample falls within defined tolerance for each populated weight class. |
| OS-045 | Similar-biome matching | Gingerbread Hearthlands maps optionally to Plains, Sunflower Plains, and Meadow; Cookie Forest maps optionally to Forest, Flower Forest, and Birch Forest. Both demonstrate several present hints without making any a hard dependency. | Main | 1/3 | Revision 13 retains both three-hint sets with empty required-hint lists; accepted fresh/reload fixed-world sampling observed 2,642 Hearthlands and 3,471 Cookie-Forest columns. A missing optional-hint fixture remains for the Sampler. |
...
| OS-047 | Temperature and downfall ranges | Gingerbread Hearthlands is the first explicit temperate selector (`0.4..1.2` temperature, `0.25..0.8` downfall); Cookie Forest narrows the wet edge with `0.15..1.5` temperature and `0.55..1.0` downfall. Later Ice-Cream Tundra, Sherbet Dunes, and Gummy Jungle supply cold, dry, warm, and wetter extremes. | Main | 1/3 | Revision 13 retains both selector contracts and fresh-world selection observes both biomes; later boundary fixtures at min/max and just outside them remain. |
| OS-048 | Surface top block | Use icing turf, sponge, crumb sand, fudge crust, and meringue signatures. Gingerbread Hearthlands uses gravity-affected Biscuit Crumbs as its warm crumb top; Cookie Forest uses Chocolate Sponge beneath its authored grove ecology. | Main | 1/3/4/5 | Revision-13 fresh and same-world reload sampling retained 1,859 Hearthlands crumb-top matches and 1,273 Cookie-Sponge top matches among 2,642 and 3,471 target-biome columns respectively, alongside green Candy, Marshmallow, Fudge, and Meringue controls. |
| OS-049 | Surface filler block and depth | Put Chocolate Sponge four blocks below the Hearthlands crumb surface, five blocks below the Cookie Forest surface, Chocolate Sponge below icing, and deeper biome-specific edible layers elsewhere. | Main | 1/3/4/5 | Revision 13 retains every 4/5-block depth; accepted fresh/reload sampling exposed 9,252 Hearthlands matching filler cells and 8,136/8,134 Cookie-Forest matching Sponge filler cells while all other dry/realm controls remained green. The two-cell Cookie difference is bounded decoration interaction, not profile drift. |
