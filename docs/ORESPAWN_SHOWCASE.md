...
| OS-006 | Forge IMC provider submission | **Verified:** keep packaged JSON as the sole distributable declaration, but conditionally build and enqueue a disabled lifecycle probe plus one selector probe during `InterModEnqueueEvent` after an isolated harness removes only the generated runtime resource. | Sampler | 7 | Released OreSpawn froze revision `6001` with exactly two owned ores and no templates. The active world snapshot retained both `source_provider=cakeworld` rules and exact `known_ores`; mutating returned provider JSON did not affect the built definition. One focused test passed and saved all dimensions. The finalizer restored packaged revision 54 and removed the test dimension immediately afterward. |
