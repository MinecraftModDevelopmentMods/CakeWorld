...
settings cannot drift. Provider revision 28 currently packages the two
...
| ID | OreSpawn capability | CakeWorld example | Placement | Slice | Verification expectation |
|---|---|---|---|---|---|
| OS-034 | Forge biome registration helper | Start simple biomes with `copyAndRegister`; Gingerbread Hearthlands, Cookie Crumb Forest, Peppermint Pinewoods, Gummy Jungle, Caramel Bogs, Sherbet Dunes, Candy-Cane Badlands, Marshmallow Peaks, Ice-Cream Tundra, Waffle Plateaus, Cupcake Gardens, Liquorice Darkwood, Lollipop Orchards, Popcorn Prairie, Soda Ocean, Custard Coast and Jellybean Archipelago demonstrate copying complete Plains, Forest, Snowy-Taiga, Bamboo-Jungle, Swamp, Desert, Badlands, Jagged-Peaks, Snowy-Plains, Savanna-Plateau, Sunflower-Plains, Dark-Forest, Birch-Forest, Plains, Ocean, Beach and Mushroom-Fields definitions while deliberately replacing climate, ambience and spawn roles. Use `blankAndRegister` only for intentionally complete advanced definitions. | Main | 1/3 | Direct registry tests prove the seventeen copied climates, including Soda Ocean `0.5/0.5`, Custard Coast `0.8/0.4` and Jellybean Archipelago `0.9/1.0`; each retains its intended inherited source roles while substituting exact CakeWorld creatures. Fixed-seed generation selects these copied biomes through the live overlay; each slice separately proves exact-save persistence. |
