...
settings cannot drift. Provider revision 48 packages two adventure templates
and the explicitly selected diagnostic Sampler. Its augment and namespace
filter plots are verified; the remaining labelled Slice 7 plots are still
wishlist work.
...
| OS-003 | `cakeworld:sampler_platter` | Never auto-selected; chosen explicitly by developers or pack makers. | **In progress:** revision 48 retains the exact Overworld augment foundation and adds selected-namespace and explicit-exclusion plots in the Nether and End. Fresh and same-save reload reproduced `66,049/1,414/64,635/50` Overworld samples plus exact `4,225/4,225` Nether conversion and `4,225/4,225` End delegation; a separate ordinary fresh world still selected `cakeworld:edible_world`. Extreme formations, aliases, retrogen, flat bedrock, intrusive test rules, and compact visual inspection remain later plots. |
...
| OS-036 | Augment placement mode | **Verified:** the explicit Sampler adds Candy Plains while retaining the delegated vanilla source. Its active palette is `augment`, `minecraft_only`, `tiny`, `coverage=0.75`, and `fallback_weight=3.0`. | Sampler | 7 | Fixed seed `5059928472718672684` sampled 66,049 positions at 32-block spacing: 1,414 Candy Plains and 64,635 Minecraft results across 50 biome IDs. Same-save reload reproduced every count exactly. |
...
| OS-038 | `selected_namespaces` replacement scope | **Verified:** the Sampler's Nether plot uses `scope=selected_namespaces` with the explicit `minecraft` allow-list, replace mode, full coverage and zero fallback. | Sampler | 7 | Fixed fresh/reload sampling converted all 4,225 vanilla-source positions to Fudge Wastes. Source namespaces not eligible for a palette delegate unchanged; the explicit Minecraft exclusion plot is verified separately under OS-040, while an unrelated-mod compatibility fixture remains OS-037's stronger gate. |
...
| OS-040 | Namespace include and exclude lists | **Verified:** the Nether plot's `include_namespaces=[minecraft]` targets vanilla source biomes, while the End plot's `scope=all` and `exclude_namespaces=[minecraft]` carves them out without a callback. | Sampler | 7 | Fixed fresh/reload sampling converted all 4,225 included Nether positions and delegated all 4,225 excluded End positions across five vanilla End IDs, with zero proposed Meringue output. |
| OS-041 | Region sizes | **Partial:** adventure palettes retain their verified broad realm settings and revision 48 now bakes live `tiny`, `small`, and `average` Sampler palettes. Large and huge labelled comparison plots remain, and the full five-size visual boundary comparison is still incomplete. | Main/Sampler | 3/7 | Packaged and active profiles agree on all three declared classes; fresh/reload output is deterministic. Do not mark the capability complete until all five 128, 256, 512, 1024, and 2048-block classes have labelled boundary evidence. |
| OS-042 | Coverage | **Verified declaration and integrated result:** all five production palettes retain `coverage=1.0`; the explicit Sampler bakes and persists `coverage=0.75`. | Main/Sampler | 1/7 | Main fixed-world output proves full replacement. The Sampler's 66,049-position fresh/reload survey produced 1,414 CakeWorld and 64,635 delegated results. Coverage and fallback interact in the effective distribution, so this observation is not presented as a standalone 75% area promise. |
...
| OS-045 | Similar-biome matching | Thirty-four copied-biome rules retain optional exact-hint lists with empty required lists; Fondant Chorus Gardens adds `minecraft:end_highlands`, while Meringue Islands deliberately retains an empty hint list to demonstrate selection by the remaining palette and climate contract. | Main | 1/2/3/4/5 | Revision 46 retains every optional hint and empty required list. Fixed fresh/reload selected Fondant through End Highlands; the Sampler keeps the separate missing-optional-hint fixture. |
...
| OS-TEST-004 | `sampler_platter` explicit selection | **In progress:** packaged and active-profile tests prove explicit selection, non-automatic metadata, the bounded augment and namespace-filter plots, exact three-realm fresh/reload distributions, and independent automatic selection of `edible_world` in an ordinary revision-48 fresh world. Completion still requires every remaining labelled diagnostic plot. |
