...
| OS-006 | Forge IMC provider submission | **Verified:** keep packaged JSON as the sole distributable declaration, but conditionally build and enqueue one disabled `cakeworld:ore/imc_probe` during `InterModEnqueueEvent` after an isolated harness removes only the generated runtime resource. | Sampler | 7 | Released OreSpawn froze revision `6001` with exactly one owned ore and no templates. The active world snapshot retained `source_provider=cakeworld`, one exact `known_ores` entry and inert dimension settings; mutating one returned provider JSON copy did not affect the built definition. One focused test passed and saved all dimensions. The Gradle finalizer restored packaged revision 54 immediately afterward. |
