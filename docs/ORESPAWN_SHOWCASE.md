...
| ID | OreSpawn capability | CakeWorld example | Placement | Slice | Verification expectation |
|---|---|---|---|---|---|
| OS-034 | Forge biome registration helper | Start simple biomes with `copyAndRegister`; Gingerbread Hearthlands demonstrates copying a complete Plains definition while deliberately replacing climate and ambient additions. Use `blankAndRegister` only for intentionally complete advanced definitions. | Main | 1/3 | Direct registry tests prove Hearthlands retains its `0.85/0.55` climate, subtitled chime, complete current Overworld role replacements, and intended structure boundaries. |
...
| OS-044 | Weighted output biomes | Gingerbread Hearthlands enters the Overworld-land palette at weight `2.5`, alongside common Candy Plains and above less-common Cookie Forest and Marshmallow Peaks; later Cupcake Gardens or Jellybean islands become the rare classes. | Main | 3 | The revision-12 profile retains each weight and a large fixed-seed sample falls within defined tolerance for each populated weight class. |
| OS-045 | Similar-biome matching | Gingerbread Hearthlands maps optionally to Plains, Sunflower Plains, and Meadow, demonstrating several present hints without making any of them a hard dependency. | Main | 3 | The revision-12 profile retains all three optional hints and a fresh fixed world produced 2,642 sampled Hearthlands columns. A missing optional-hint fixture remains for the Sampler. |
...
| OS-047 | Temperature and downfall ranges | Gingerbread Hearthlands is the first explicit temperate selector (`0.4..1.2` temperature, `0.25..0.8` downfall); later Ice-Cream Tundra, Sherbet Dunes, and Gummy Jungle supply cold, dry, warm, and wet extremes. | Main | 3 | The revision-12 profile retains the Hearthlands bounds and fresh-world selection observed the biome; later boundary fixtures at min/max and just outside them remain. |
| OS-048 | Surface top block | Use icing turf, sponge, crumb sand, fudge crust, and meringue signatures. Gingerbread Hearthlands uses gravity-affected Biscuit Crumbs as its warm crumb top. | Main | 1/3/4/5 | Revision-12 fresh/reload sampling retained 1,859 matching natural Hearthlands crumb tops among 2,642 biome columns, alongside the existing Candy, Cookie, Marshmallow, Fudge, and Meringue controls. |
| OS-049 | Surface filler block and depth | Put Chocolate Sponge four blocks below the Hearthlands crumb surface, Chocolate Sponge below icing, and deeper biome-specific edible layers elsewhere. | Main | 1/3/4/5 | The revision-12 profile retained every 4/5-block depth; fresh/reload sampling found 9,253 matching Hearthlands Sponge filler cells while all other dry/realm controls remained green. |
