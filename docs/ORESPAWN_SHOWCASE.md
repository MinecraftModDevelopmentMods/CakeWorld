...
settings cannot drift. Provider revision 44 currently packages the three
...
| ID | OreSpawn capability | CakeWorld example | Placement | Slice | Verification expectation |
|---|---|---|---|---|---|
| OS-034 | Forge biome registration helper | Start simple biomes with `copyAndRegister`; the thirty-one previously verified CakeWorld biomes copy complete vanilla source definitions rather than recreating hidden generation state. Macaron Archipelago adds the thirty-second definition by copying End Highlands and replacing climate, ambience and spawn roles while retaining the source's generation contract. Use `blankAndRegister` only for intentionally complete advanced definitions. | Main | 1/2/3/4/5 | Direct registry tests prove all thirty-two copied climates, including all seven Nether outputs and all five End outputs. Fixed-seed generation selects each copied biome through the live three-dimensional overlay and preserves exact-save identity; Macaron's End-Highlands definition returned around the natural Wayfinder at `811,59,778`. |
...
| OS-044 | Weighted output biomes | Gingerbread Hearthlands enters the Overworld-land palette at weight `2.5`, alongside Candy Plains and the previously verified complete land, ocean and cave palettes. The Nether palette selects Burnt-Toffee Deltas at `1.5`, Cinnamon Ember Groves at `1.25`, Treacle Soul Valleys at `1.2`, Black-Liquorice Labyrinths at `1.1`, Fudge Wastes at `1.0`, Molten-Marshmallow Calderas at `0.8` and Chilli-Chocolate Crags at `0.65`. The End palette selects Macaron Archipelago at `0.75`, Meringue Islands at `1.0`, Starlight Sugar Fields at `1.25`, Mooncake Barrens at `1.5` and Candyfloss Cloudbanks at `2.0`; their source families are disjoint, so these remain profile weights rather than global area-ratio promises. | Main | 2/3/4/5 | Revision 44 retains every populated cave, land, ocean, Nether and five-biome End weight. The normal and BaseMetals templates are generated from the same canonical profile and differ only by the thirteen declared compatibility ores. Weight affects overlapping candidates; it is not falsely presented as a global rarity guarantee for disjoint source families. |
| OS-045 | Similar-biome matching | The thirty previously verified copied-biome hint rules remain optional rather than becoming hard dependencies. Macaron Archipelago adds the thirty-first optional exact hint, `minecraft:end_highlands`; Meringue Islands continues to leave its hint list empty to demonstrate selection by the remaining palette and climate contracts. Every required-hint list remains empty in the adventure. | Main | 1/2/3/4/5 | Revision 44 retains all thirty-one optional copied-biome hint rules with empty required-hint lists. Fixed-seed generation selected Macaron through End Highlands around the natural Wayfinder, while the separate missing-optional-hint fixture remains for the Sampler. |
...
| OS-047 | Temperature and downfall ranges | The thirty-one previously verified selectors retain their complete source-inclusive or deliberately narrow climate windows. Macaron Archipelago adds the thirty-second selector: the complete End-Highlands source range (`-2.0..2.0` temperature, `0.0..1.0` downfall) mapped to a registered `0.5/0.0` output, matching the other four End dreamscapes without narrowing away valid source cells. | Main | 1/2/3/4/5 | Revision 44 retains all thirty-two selector contracts. Fixed-seed generation selected Macaron at an End-Highlands-derived location and retained the registered climate. Exact boundary fixtures for every range remain, including the prior Waffle proof that selectors evaluate source rather than output climate. |
| OS-048 | Surface top block | The adventure uses distinct registered top materials for every surface biome while the three underground biomes deliberately omit the surface object. Revision 44 adds Macaron Shell beside the already verified Meringue Foam, Candyfloss Cloud, Mooncake Crust and Starlight Sugar Grass End signatures. | Main | 1/3/4/5 | Direct tests retain every named top material and all three cave omissions. Fixed fresh evidence retained `5,487` independent Macaron-Shell cells around the natural Wayfinder; earlier slice evidence retains each other top-material count and literal-source negative. |
| OS-049 | Surface filler block and depth | Every surface output declares an edible filler and exact depth. Revision 44 adds three-deep Marshmallow below Macaron Shell, alongside five-deep Marshmallow below Meringue Foam, four-deep Candyfloss Cloud below Cloudbanks, five-deep Biscuit Stone below Mooncake Crust and four-deep Rock Candy below Starlight Sugar Grass. | Main | 1/3/4/5 | Direct tests prove every exact declaration. Fixed fresh evidence retained `16,374` independent Marshmallow cells in the bounded Macaron audit. Counts are bounded observations; configured depth three is the exact contract. |
| OS-050 | Underwater surface block | Retain the eighteen previously verified biome-specific wet-edge materials and add Rock Candy beneath Macaron Archipelago, keeping underwater presentation explicit even in an ordinarily dry End source family. | Main | 3/5 | Revision 44 retains all nineteen live fields and direct profile tests retain every control. This declaration remains independent of the separately shelved automatic `ice_block` runtime in OS-056. |
...
| OS-102 | Surface-before-child-feature ordering | OreSpawn 4.0.6 applies provider-owned exposed-ground surfaces during `LOCAL_MODIFICATIONS`, before CakeWorld's ordinary decoration features author landmarks and vegetation. CakeWorld features therefore search bounded in-chunk safe sites, accept declared edible supports and relief, fill only clear support gaps, and reject fluids, block entities, ores, structures and unknown or player-authored solids. Ordering is not a resource-protection contract: structural landmark blocks remain excluded from managed host tags where required. Burnt-Toffee Deltas and Molten-Marshmallow Calderas retain their filtered copied-feature controls. The End now supplies five independent one-chunk cases—Meringue Star Landing, Cloudstep Lookout, Crumb Moon Dial, Sugar-Star Observatory and Macaron Wayfinder—while the larger Crater Kitchen retains its nine-point Mooncake biome/support predicate. The Observatory proves an exact horizontal telescope state survives rotation; the Wayfinder independently keeps four flavour arms aligned to world-cardinal directions while its lamp-and-kitchen silhouette rotates. Neither owns an entity, block entity, inventory, marker, repair or replay path, while optional Starsteel remains independently managed geology rather than authored scenery. | Main | 2/3/4/5 | Published release `4.0.6.118021`, CurseForge file `8688546`, remains the tested dependency. Revision 44 passed direct target tests plus fixed fresh/reload evidence. The Wayfinder returned at `811,59,778`, `COUNTERCLOCKWISE_90`, with exact fresh `49/36/3/3/3/3/1/4/4/4/4/4/1/1` and reload `49/36/3/3/3/3/1/4/4/4/4/3/1/1+Brick` identity; both runs retained the same cardinal compass and `683,940/5,487/16,374/98,374/53,965` surrounding biome/Shell/Marshmallow/Rock-Candy/Nougat counts. Earlier slice evidence retains the Observatory, Dial, Crater Kitchen and every named surface landmark. Direct tests cover placement bounds, support, relief, fluid, block-entity, obstacle, chunk containment, rotation and cardinal controls. No OreSpawn source or local JAR was changed. |
