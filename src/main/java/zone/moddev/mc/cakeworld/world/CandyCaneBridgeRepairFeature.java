...
/**
 * Restores bridge slices after surface decoration.
 */
public final class CandyCaneBridgeRepairFeature
		extends Feature<NoneFeatureConfiguration> {
	public static final ResourceLocation ID =
			new ResourceLocation(CakeWorld.MODID,
					"candy_cane_bridge_repair");
	public static final CandyCaneBridgeRepairFeature FEATURE =
			new CandyCaneBridgeRepairFeature();
	...
	private CandyCaneBridgeRepairFeature() {
	...
	public boolean place(
		...
		ConfiguredStructureFeature<?, ?> bridge =
				world.registryAccess()
						.registryOrThrow(Registry
								.CONFIGURED_STRUCTURE_FEATURE_REGISTRY)
						.get(CandyCaneBridgeFeature
								.STRUCTURE_ID);
		if (bridge == null) {
			return false;
		}
		...
		for (StructureStart start : starts) {
			BoundingBox bridgeBounds =
					start.getBoundingBox();
			if (!start.isValid()
					|| !bridgeBounds.intersects(
			...
			BlockPos centre = new BlockPos(
					...
					bridgeBounds.minZ()
							+ CandyCaneBridgeStructureFeature
									.CENTRE_OFFSET);
			CandyCaneBridgeFeature.rebuildInBounds(
