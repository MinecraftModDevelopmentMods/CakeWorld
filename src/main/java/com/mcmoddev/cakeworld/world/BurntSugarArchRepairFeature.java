...
/**
 * Restores Burnt-Sugar Arch slices after terrain decoration.
 */
public final class BurntSugarArchRepairFeature
		extends Feature<NoneFeatureConfiguration> {
	public static final ResourceLocation ID =
			new ResourceLocation(CakeWorld.MODID,
					"burnt_sugar_arch_repair");
	public static final BurntSugarArchRepairFeature FEATURE =
			new BurntSugarArchRepairFeature();
	...
	private BurntSugarArchRepairFeature() {
	...
	public boolean place(
		...
		ConfiguredStructureFeature<?, ?> arch =
				world.registryAccess()
						.registryOrThrow(Registry
								.CONFIGURED_STRUCTURE_FEATURE_REGISTRY)
						.get(BurntSugarArchFeature
								.STRUCTURE_ID);
		if (arch == null) {
			return false;
		}
		...
		for (StructureStart start : starts) {
			BoundingBox archBounds =
					start.getBoundingBox();
			if (!start.isValid()
					|| !archBounds.intersects(
			...
			BlockPos centre = new BlockPos(
					...
					archBounds.minY()
							+ BurntSugarArchStructureFeature
									.SURFACE_OFFSET,
					archBounds.minZ() + 8);
			BurntSugarArchFeature.rebuildInBounds(
